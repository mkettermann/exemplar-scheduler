...
/**
 * Boot e encerramento do processo, fora do entrypoint para poderem ser
 * testados sem subir nada. `server.ts` só liga os sinais e chama `iniciar`.
 * Ver `docs/05-scheduler.md`, seção "O entrypoint: boot e encerramento".
 */
let servidor: FastifyInstance | undefined;
let encerrando = false;

/** Banco, jobs e HTTP, nessa ordem. */
...
export async function encerrar(sinal: string): Promise<void> {
  ...
  try {
    ...
    logger.error({ err: error_ }, 'Falha durante o encerramento');
    process.exit(1);
  }
}
